<template>
	<el-card>
		...
		<el-table v-loading="state.dataListLoading" :data="state.dataList" border style="width: 100%" @selection-change="selectionChangeHandle">
			...
			<el-table-column prop="kpName" label="知识点" show-overflow-tooltip header-align="center" align="center" width="150"></el-table-column>

			<fast-table-column prop="isTypical" label="典型" dict-type="yes_or_no" header-align="center" align="center"></fast-table-column>
			<el-table-column prop="sampleNum" label="样例数" header-align="center" align="center"></el-table-column>
          ...
          <span  v-if="scope.row.status === 1">
            <el-button style="padding: 0" type="text" size="small" @click="updateStatus(scope.row)">下架</el-button>
            <el-divider direction="vertical" />
					  <el-button
                style="padding: 0"
                type="text"
                size="small"
                @click="problemTestHandle(scope.row)"
            >测试</el-button>
          </span>
          <span v-else>
						<el-button style="padding: 0" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">编辑</el-button>
						<el-divider direction="vertical" />
						<!--						<el-button style="padding: 0" type="text" size="small" @click="editResources(scope.row.id, 3)">资源管理</el-button>-->
						<el-button style="padding: 0" type="text" size="small" @click="editSample(scope.row.id)">样例管理</el-button>
						<el-divider direction="vertical" />
						<el-button style="padding: 0" type="text" size="small" @click="updateStatus(scope.row)">发布</el-button>
					</span>
					<el-divider direction="vertical" />
					<el-button
						style="padding: 0"
						type="text"
						size="small"
						@click="router.push({ path: '/problem/problem-explanation/' + scope.row.id + '/' + 3 })"
						>题解管理</el-button
					>
...
<!--    题目测试-->
    <problem-test ref="problemTestRef"></problem-test>
	</el-card>
</template>

<script setup lang="ts" name="ProblemCode-problemIndex">
...
import { useCodeProblemUpdateStatusApi } from '@/api/problem/codeproblem'
import KpSelectTree from '@/components/kp-select-tree/kp-select-tree.vue'
import { dataListUrl, deleteUrl } from '@/api/problem/codeproblem'
import ProblemResource from '@/views/problem/problem-resource/problem-resource.vue'
import ImportExcel from '@/views/problem/code-problem/import-excel.vue'
import { ElMessage } from 'element-plus'
import ProblemTest from './problem-test.vue'
...
const editSample = (id?: number) => {
	router.push({ path: '/problem/sample/' + id })
}
const updateStatus = (row: any) => {
	if (row.sampleNum <= 0) {
		ElMessage.warning('请至少添加一个样例后发布！')
		return
	}
	useCodeProblemUpdateStatusApi(row.id).then(res => {
...
const problemTestRef = ref()
const problemTestHandle = (row:any) => {
  problemTestRef.value.init(row.id, 3, 4, 0,0)
}
