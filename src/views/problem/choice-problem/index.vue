	...
	<el-card>
      ...
      <el-form-item>
		...
		<el-table v-loading="state.dataListLoading" :data="state.dataList" border style="width: 100%" @selection-change="selectionChangeHandle">
			...
			<el-table-column prop="optionNum" label="选项数" header-align="center" align="center"></el-table-column>
			...
			<el-table-column label="操作" fixed="right" header-align="center" align="center" width="290">
				<template #default="scope">
					<el-button v-if="scope.row.status === 1" style="padding: 0" type="text" size="small" @click="updateStatus(scope.row)">下架</el-button>

					<span v-else>
						<el-button style="padding: 0" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">编辑</el-button>
						<!--						<el-button style="padding: 0" type="text" size="small" @click="editResources(scope.row.id, 1)">资源管理</el-button>-->
					...
					<el-button
						...
						type="text"
						size="small"
						@click="router.push({ path: '/problem/problem-explanation/' + scope.row.id + '/' + 1 })"
						>题解管理</el-button
					>
		...
		<choice-option ref="choiceOptionRef" v-model:list="list" @refreshDataList="getDataList"></choice-option>

		<!--资源管理弹窗-->
		<problem-resource ref="editResourceRef"></problem-resource>

    <import-excel ref="importExcelRef"></import-excel>
	</el-card>
</template>

<script setup lang="ts" name="ProblemChoice-problemIndex">
import { useCrud } from '@/hooks'
import { provide, reactive, ref } from 'vue'
import AddOrUpdate from './add-or-update.vue'
import { IHooksOptions } from '@/hooks/interface'
import { ElMessage } from 'element-plus'
import { useChoiceProblemUpdateStatusApi } from '@/api/problem/choiceproblem'
import KpSelectTree from '@/components/kp-select-tree/kp-select-tree.vue'
import { dataListUrl, deleteUrl } from '@/api/problem/choiceproblem'
...
provide('dataList', list)
const choiceOptionRef = ref()
...
const updateStatus = (row: any) => {
	if (row.optionNum <= 0) {
		ElMessage.warning('请至少添加一个选项后发布！')
		return
	}
	useChoiceProblemUpdateStatusApi(row.id).then(res => {
