<template>
	<el-card>
		...
		<el-form :inline="true">
			...
			<el-form-item v-if="state.dataListSelections.length > 0">
			...
			<el-form-item v-auth="'sys:user:import'">
				<el-upload :action="constant.uploadUserExcelUrl" :before-upload="beforeUpload" :on-success="handleSuccess" :show-file-list="false">
					<el-button><svg-icon icon="icon-upload"></svg-icon>导入</el-button>
				</el-upload>
			</el-form-item>
			<el-form-item>
				<el-button v-auth="'sys:user:export'" @click="downloadExcel()"><svg-icon icon="icon-download"></svg-icon>导出</el-button>
			</el-form-item>
		</el-form>
...
import { dataListUrl, deleteUrl, useUserExportApi } from '@/api/sys/user'
