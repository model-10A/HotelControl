<template>
	<div>
		...
		<el-table :data="state.dataList" :max-height="800">
			<el-table-column align="center" header-align="center" label="学号" prop="studentNumber" show-overflow-tooltip width="120"> </el-table-column>
			...
			<el-table-column align="center" header-align="center" label="评价至老师" prop="teaEvaluation" show-overflow-tooltip> </el-table-column>
			<el-table-column align="center" header-align="center" label="点赞数" prop="likesNum" width="80">
				<template #default="scope">
					{{ scope.row.likesNum != null ? scope.row.likesNum : 0 }}
				</template>
			</el-table-column>
			<el-table-column align="center" fixed="right" header-align="center" label="操作" width="300px">
				<template #default="scope">
					<el-button size="small" type="text" @click="showEvaluation(scope.row)">评价</el-button>
					...
					<el-button v-show="scope.row.rollCall === 1" size="small" type="text" @click="cancelCallStudentsHandle(scope.row, false)"
						>取消点名</el-button
					>
					<el-button v-show="scope.row.status === 0" size="small" type="text" @click="cancelCallStudentsHandle(scope.row, true)">取消请假</el-button>
					<el-button v-show="scope.row.status !== 0" size="small" type="text" @click="callStudentsHandle(scope.row, true)">请假</el-button>

					<el-button :disabled="likesDisabled" size="small" type="text" @click="giveOrResetLikes(scope.row, 1)">点赞</el-button>
					<el-button :disabled="likesDisabled" size="small" type="text" @click="giveOrResetLikes(scope.row, 0)">重置点赞</el-button>
				</template>
			</el-table-column>
		</el-table>
	...
	<el-dialog v-model="evaluationVisible" title="评价" width="50%">
		<el-form-item label="评价至教务">
			<el-col :span="20">
				<el-input v-model="sysEvaluation" type="textarea"></el-input>
			</el-col>
		</el-form-item>
		<el-form-item label="评价至老师">
			<el-col :span="20">
				<el-input v-model="teaEvaluation" type="textarea"></el-input>
			</el-col>
		</el-form-item>
		<template #footer>
...
const emit = defineEmits(['pushLessonDestroyedRecord'])
